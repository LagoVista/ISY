...
namespace LagoVista.SmartThings
{
    public class SmartThingsHubs
    {
        ...
        {
            var json = await Core.PlatformSupport.Services.Storage.GetKVPAsync<String>(HUBS_JSON);
        ...
        {
            await Core.PlatformSupport.Services.Storage.StoreKVP(HUBS_JSON, JsonConvert.SerializeObject(_hubs));
        }
